...
internal class SagaDispatcher : DispatcherBase
{
    ...
    private void AddSagaRelationNetwork(Type eventBusHandlerType)
    {
        ...
        foreach (var handler in eventHandlers)
        {
            ...
            base.AddRelationNetwork(handler.EventType, handler);
        }
    }
    ...
    private List<EventHandlerAttribute> GetSagaHandlers(Type eventBusHandlerType)
    {
        ...
        foreach (var method in methods)
        {
            ...
            if (parameters != null && parameters.Length == 1 && parameters.All(parameter => typeof(IEvent).IsAssignableFrom(parameter.ParameterType)) && IsSagaMode(eventBusHandlerType, method))
            {
