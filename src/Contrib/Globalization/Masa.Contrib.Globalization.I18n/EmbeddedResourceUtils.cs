...
public class EmbeddedResourceUtils
{
    ...
    public List<KeyValuePair<Assembly, string[]>> GetResources(string resourcesDirectory)
    {
        ...
        foreach (var item in _list)
        {
            var data = item.Value.Where(resourceName
                => resourceName.IndexOf(FormatResourcesDirectory(resourcesDirectory), StringComparison.OrdinalIgnoreCase) > 0).ToArray();
            if (data.Length > 0)
            {
    ...
    public Stream? GetStream(Assembly assembly, string fileName) => assembly.GetManifestResourceStream(fileName);

    public string? GetCulture(string resourcesDirectory, string fileName)
    {
