...
public class DaprConfigurationApiClient : Masa.Contrib.Configuration.ConfigurationApi.Dcc.Internal.ConfigurationApiBase, IConfigurationApiClient
{
    ...
    public async Task<T> GetAsync<T>(string environment, string cluster, string appId, string configObject, Action<T>? valueChanged = null)
    {
        ...
        var value = await _taskJsonObjects.GetOrAdd(key, k => new Lazy<Task<object>>(async () =>
        {
            ...
            {
                var deserialized = JsonSerializer.Deserialize<T>(value, _dynamicJsonSerializerOptions);
...
#pragma warning restore CS0618
                ...
                await foreach (var configurationItems in subscribeResponse.Source.WithCancellation(cts.Token).ConfigureAwait(false))
                {
                    ...
                    foreach (var (configKey, configItem) in configurationItems)
                    {
                        ...
                        var model = JsonSerializer.Deserialize<PublishReleaseModel>(configItem.Value, _jsonSerializerOptions);
                        var result = FormatRaw(model, key);
