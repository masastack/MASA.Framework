...
public class BackgroundJobOptionsTest
{
    ...
    public void TestBackgroundJobOptions()
    {
        ...
        Assert.ThrowsExactly<MasaArgumentException>(() => backgroundJobOptions.BatchSize = 0);
        Assert.ThrowsExactly<MasaArgumentException>(() => backgroundJobOptions.BatchSize = -1);

        Assert.AreEqual(5, backgroundJobOptions.PollInterval);
        Assert.ThrowsExactly<MasaArgumentException>(() => backgroundJobOptions.PollInterval = 0);
        Assert.ThrowsExactly<MasaArgumentException>(() => backgroundJobOptions.PollInterval = -1);
        ...
        Assert.ThrowsExactly<MasaArgumentException>(() => backgroundJobOptions.MaxRetryTimes = -1);

        backgroundJobOptions.FirstWaitDuration = 1;
        Assert.AreEqual(1, backgroundJobOptions.FirstWaitDuration);
        Assert.ThrowsExactly<MasaArgumentException>(() => backgroundJobOptions.FirstWaitDuration = 0);
        Assert.ThrowsExactly<MasaArgumentException>(() => backgroundJobOptions.FirstWaitDuration = -1);

        backgroundJobOptions.WaitDuration = 1;
        Assert.AreEqual(1, backgroundJobOptions.WaitDuration);
        Assert.ThrowsExactly<MasaArgumentException>(() => backgroundJobOptions.WaitDuration = 0);
        Assert.ThrowsExactly<MasaArgumentException>(() => backgroundJobOptions.WaitDuration = -1);
    }
}
